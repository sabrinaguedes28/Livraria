...
<body>
    ...
    <nav class="white" role="navigation ">
      <div class="nav-wrapper container">
        ...
        <ul class="right hide-on-med-and-down">
          ...
          <li><a class="black-text" href="index.php">Início</a></li>
        </ul>
  ...
  <div id="index-banner" class="parallax-container" style="height: 400px;">
    <div class="section no-pad-bot">
      <div class="container">
        <br><br>
        <h1 style="text-transform: uppercase;" class="header center teal-text text-lighten-2"><?php echo $tmpString;?></h1>
        <div class="row center">
          <h5  class="header col s12 light">Lista completa dos Produtos aqui!</h5>
        </div>
      </div>
    </div>
    <div class="parallax"><img style="filter: brightness(0.3);" src="img/dark.jpg" alt="Unsplashed background img 1"></div>
  </div>
...
$i = 1;
foreach($dados['components'] as $id => $valor){
    if ($dados['components'][$id]['title'] == $tmpString){
       $here = $dados['components'][$id];
    }
}
...
foreach($item as $key => $value) {
    if($i <= 9){
        foreach ($here['products'] as $lista){
            if ($key == $lista['id']){
    echo '
